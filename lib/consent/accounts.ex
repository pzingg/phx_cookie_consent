defmodule Consent.Accounts do
  ...
  def get_user_consent(%User{} = user) do
    user = Repo.preload(user, :consent, force: true)
    user.consent
  end
  ...
  def delete_user_consent(%User{} = user) do
    results =
      user
      |> User.consent_changeset(%{consent: nil})
      |> Repo.update()

    case results do
      {:ok, user} -> user.consent
